...
const UserService = {
    ...
    addUser: async ( username, password, age,) => {
        ...
        if(query.length<=0){
           ...
           return UserModel.create({username,password,age}).catch(err=>{// 在此处 捕获处理 异常
    ...
    UpdateUser:async (_id,username,password,age)=>{
        ...
        // UserModel.updateOne() 如果数据库 集合中找到多项 匹配_id的 只会修改更新第一项, 而 UserModel.updateMany() 是会修改更新多多项
         return UserModel.updateOne({_id:_id},{$set:{username,password,age}}).catch(err=>{// 在此处 捕获处理 异常
